...
class PostState(BaseState):
    ...
    # Rename board_id to current_board_id to avoid conflict with route parameter
    current_board_id: Optional[str] = None  # Changed from board_id
    ...
    async def load_board_and_posts(self):
        ...
        print(f"Current board ID: {self.current_board_id}")
        self.is_loading = True
        yield

        # Use current_board_id instead of board_id
        ...
        try:
            board_res = self.supabase_client.from_("boards").select("*").eq("id", self.current_board_id).single().execute()
            self.board_name = board_res.data.get("name", "알 수 없는 게시판")
            self.board_description = board_res.data.get("description", "")

            posts_res = self.supabase_client.from_("posts").select("*").eq("board_id", self.current_board_id).order("created_at", desc=True).execute()
    ...
    async def handle_search(self):
        ...
        try:
            response = self.supabase_client.from_("posts").select("*") \
                .eq("board_id", self.current_board_id) \
    ...
    async def create_post(self, form_data: dict):
        ...
        try:
            ...
            self.supabase_client.from_("posts").insert({
                ...
                "board_id": self.current_board_id,  # Updated to current_board_id
                "author_email": auth_state.user.email,
            }).execute()

            yield PostState.load_board_and_posts
            yield rx.redirect(f"/boards/{self.current_board_id}")
        except Exception as e:
...
class PostDetailState(BaseState):
    ...
    # [FIXED] Explicitly define the dynamic route variable.
    ...
    async def load_post(self):
        ...
        # self.post_id is automatically populated from the URL.
        ...
        try:
            response = self.supabase_client.from_("posts").select("*").eq("id", self.current_post_id).single().execute()
